...
| `PYTORRENT_CORS` | `http://localhost:5173` | Comma-separated **allowed browser origins** for the API (cookies + fetch). Add your public site origin when hosting the UI separately (e.g. `https://app.example.com,http://127.0.0.1:8765`). |
...
- **Home experience:** Multiple YTS rails (trending, recent, genre slices via **`GET /browse/yts/list`**, classics by year/rating). The **hero** rotates through top trending titles. **Continue watching** resumes in-browser playback (localStorage + optional **`/user/watch/progress`** when signed in).
- **Accounts (optional):** SQLite at **`$PYTORRENT_DATA_DIR/pytorrent_users.db`** — `POST /auth/register`, `POST /auth/login` (sets **`pt_session`** cookie), `POST /auth/logout`, `GET /auth/me`. **`GET`/`PUT /user/settings`** stores dashboard JSON (`favoriteGenres`, `hiddenRowKeys`, `showRecommendations`). Guests can still use **Dashboard** in the UI; prefs are saved in **localStorage** only. Torrent data and jobs remain **on the daemon host**; accounts mainly sync UI preferences and watch progress.
- **Publishing:** The stack is still **local-first** (the BitTorrent engine runs where `pytorrentd` runs). To expose the UI publicly, reverse-proxy to the daemon, set **`PYTORRENT_CORS`** to your frontend origin(s), use **HTTPS**, and understand **security** (no built-in rate limits; protect the API if it is not on localhost).
